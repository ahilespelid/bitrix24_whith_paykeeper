...
class PayKeeperHandler extends PaySystem\ServiceHandler implements PaySystem\IRefund
{
    ...
    public function cart($order)
    {
        ...
        if (!$this->checkDeliveryIncluded($this->getShippingPrice(), $delivery_name)
            ...
            $this->setUseDelivery();
            $this->updateFiscalCart($this->getPaymentFormType(), $delivery_name, $delivery_price, 1, $delivery_price, $delivery_taxes["tax"],"service");
	...
	public function processRequest(Payment $payment, Request $request)
	{
        ...
        $secret = \Bitrix\Sale\BusinessValue::get(
            "PAYKEEPER_SECRET",
            "PAYSYSTEM_".$payment->getField('PAY_SYSTEM_ID'),
            $payment->getOrder()->getField('PERSON_TYPE_ID')
        );

        $status_id_after_payment = \Bitrix\Sale\BusinessValue::get(
            "PAYKEEPER_STATUS_AFTER_PAYMENT",
            "PAYSYSTEM_".$payment->getField('PAY_SYSTEM_ID'),
            $payment->getOrder()->getField('PERSON_TYPE_ID')
        );
